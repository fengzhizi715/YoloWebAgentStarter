...
export function AgentContextPicker({ locale, profile, context, disabled, onChange }: {
  ...
  return <section className="agent-binding-panel" aria-label={locale === "zh" ? "助手模式与对象" : "Assistant mode and context"}>
    <div className="agent-profile-options">{PROFILE_IDS.map((id) => <button type="button" className="button" key={id}
      aria-pressed={profile === id} disabled={disabled} onClick={() => onChange(id,
        id === "global" ? {} : id === "dataset" ? { dataset_id: context.dataset_id } : id === "training"
          ...
          : { dataset_id: context.dataset_id, model_id: context.model_id })}>{profileTitle(id, locale)}</button>)}</div>
    <div className="agent-context-selectors">
      <label>{locale === "zh" ? "数据集" : "Dataset"}<select disabled={disabled} value={context.dataset_id ?? ""}
        onChange={(event) => onChange(event.target.value ? "dataset" : "global", event.target.value ? { dataset_id: event.target.value } : {})}>
      ...
      <label>{locale === "zh" ? "训练任务" : "Training task"}<select disabled={disabled} value={context.training_task_id ?? ""}
      ...
      <label>{locale === "zh" ? "模型" : "Model"}<select disabled={disabled} value={context.model_id ?? ""}
