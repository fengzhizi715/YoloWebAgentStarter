...
export default function App() {
  ...
  const [quality, setQuality] = useState<DatasetQualityReport>();
    ...
    <StarterShell active={activeSection} datasetName={selected?.name} onNavigate={navigate}>
      ...
      {view === "annotation" && displayedDataset && selectedImage ? (
        ...
        <DatasetDetail
          ...
          onQuality={() => selected && run(async () => setQuality(await api.qualityReport(selected.id)))}
...
function DatasetDetail(props: {
  selected: Dataset; classes: ClassLabel[]; images: ImageItem[]; report?: ValidationReport; quality?: DatasetQualityReport; duplicates?: DuplicateReport; activeClassId: string; busy: boolean;
  onBack: () => void; onAddClass: (name: string) => void;
  onUpload: (files: File[], split: SplitName) => void; onScan: (path: string, split: SplitName) => void; onValidate: () => void; onQuality: () => void; onDuplicates: () => void; onVideo: (file: File, split: SplitName, interval: number) => void;
  ...
  return (
    <main className="dataset-detail-page">
      ...
      <section className="workspace panel">
          ...
          {props.quality && <QualityPanel report={props.quality} />}
...
function QualityPanel({ report }: { report: DatasetQualityReport }) { return <section className="quality-panel"><div className="quality-summary"><span><small>标注覆盖率</small><strong>{Math.round(report.summary.coverage * 100)}%</strong></span><span><small>标注总数</small><strong>{report.summary.annotation_count}</strong></span><span><small>小目标</small><strong>{report.summary.small_object_count}</strong></span><span><small>问题</small><strong>{report.issues.length}</strong></span></div><div className="quality-grid"><div><h3>类别分布</h3>{report.class_distribution.map((item) => <p key={item.class_id}><span>{item.class_index}. {item.name}</span><b>{item.count}</b><i><em style={{ width: `${Math.max(item.ratio * 100, item.count ? 3 : 0)}%` }} /></i></p>)}</div><div><h3>需要关注</h3>{report.issues.slice(0, 5).map((item, index) => <p key={`${item.type}-${index}`}><b>{item.level}</b><span>{item.message}</span></p>)}{!report.issues.length && <p>当前未发现需要关注的问题。</p>}</div></div></section>; }
