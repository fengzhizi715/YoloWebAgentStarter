  ...
  return (
    <main className="models-layout">
      ...
      <div className="models-grid">
        ...
        {selected ? <section className="panel model-detail-panel"><div className="detail-head"><div><span className="eyebrow">MODEL DETAIL</span><h2>{selected.name}</h2><p className="muted">{selected.format.toUpperCase()} · {selected.task_type} · {selected.source}</p></div><span className={`status ${selected.status}`}>{selected.status === "active" ? "启用" : "已归档"}</span></div><div className="model-actions"><a className="button" href={api.downloadModelUrl(selected.id)}>下载 {selected.format.toUpperCase()}</a>{selected.format === "pt" && <button className="button primary" disabled={busy} onClick={exportOnnx}>导出 ONNX FP32</button>}<button className="button" disabled={busy} onClick={toggleArchive}>{selected.status === "active" ? "归档" : "恢复"}</button><button className="button danger" disabled={busy} onClick={remove}>删除</button></div><div className="model-form"><label>名称<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>版本<input value={version} onChange={(event) => setVersion(event.target.value)} /></label><label>备注<textarea value={notes} onChange={(event) => setNotes(event.target.value)} /></label><button className="button" disabled={busy || !name.trim() || !version.trim()} onClick={save}>保存元数据</button></div><div className="model-meta"><span>文件：{selected.model_path}</span><span>基础模型：{selected.base_model || "—"}</span><span>mAP50：{selected.map50 ?? "—"}</span><span>创建：{selected.created_at.slice(0, 16).replace("T", " ")}</span></div></section> : <section className="panel model-detail-panel empty-state"><strong>选择一个模型查看详情</strong></section>}
      </div>
    </main>
  );
}
