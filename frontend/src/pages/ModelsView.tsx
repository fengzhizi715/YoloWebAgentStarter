...
export function ModelsView({ dataset, onBack, onOpenPreannotated }: Props) {
  ...
  const [evaluationSplit, setEvaluationSplit] = useState<SplitName>("val");
  ...
  const evaluate = () => selected && run(async () => { const record = await api.evaluateModel(selected.id, evaluationSplit); setEvaluations((items) => [record, ...items]); });
