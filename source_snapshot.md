...
- `backend/app/training/runtime/device_service.py` 的本地设备发现、CUDA 选择和多 GPU `device=0,1` 契约；Starter 将其重建为本地单机服务，并以 Starter 自有的进程内互斥门串行化训练与自动标注，不引入上游资源调度器。
- `backend/app/settings/service.py`、`schemas.py` 与 `logs/service.py` 的可见工作流契约；Starter 只提供独立的本地 SAM 设置、语言偏好和后端运行日志边界。

Starter 对这些能力保留独立的 SQLite schema、存储边界、路由和前端实现；不复制 Enterprise 的设置/Profile 引擎、Vision Provider、Agent、文本分割或 deployment 模块，也不在运行时读取源仓库。数据集级自动标注按上游任务/日志/人工审核契约在 Starter 内独立重建，仅使用本地受管 Ultralytics PT；类别只会按同名自动映射，其余映射必须由用户确认。SAM 设置和运行日志属于 Starter 的最小本地边界，不依赖 Enterprise 设置/Profile 运行时。
